 ...
 */
class Works extends Ad_layout
{
	var $table = "tbl_tuyendung"; 
	...
	public function index()
	{
		...
		$config['base_url'] = base_url().'/admincp/works';
		...
		$this->_data['ad_main_content'] = $this->load->view('admincp/works/index', array(
				'content' => $result['faqs'],
	...
	public function add()
	{
		...
		$this->_data['ad_main_content'] = $this->load->view('admincp/works/add', array(			
			'error' => @$error,
		), true);
		$this->load->view('admincp/ad_layout_view', $this->_data);
	}
		...
		if($insert){
			echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'>
						...
						alert('Bạn đã thêm mới thành công !');
						 location.assign('".base_url('admincp/works.html')."');
					  </script>";
		}else {
			echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'>
						<script>
	...
	public function edit()
	{
		...
		if ($this->input->post () != "") {
			$keypage = $this->utility_model->find_slugwhere ( $this->table, 'seo_name', $this->input->post ( 'keypage' ) );
			$checkslug = $this->works_model->checkSlug($this->input->post ( 'newsID' ), $this->input->post ( 'keypage' ) );
			if($checkslug == TRUE){
				$this->updated ();
				$error = '';
			}else {
		...
		$this->_data ['ad_main_content'] = $this->load->view ( 'admincp/works/edit', array (
				'error' => @$error,
