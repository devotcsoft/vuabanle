...
class Faqs extends Ad_layout
{
	...
	public function index()
	{
		...
		$result = $this->faqs_model->getAllSearch($arraySearch, $page, $config['per_page']);
		...
		$this->_data['ad_main_content'] = $this->load->view('admincp/faqs/index', array(
				'content' => $result['faqs'],
	...
	public function add()
	{
		...
		$this->_data['ad_main_content'] = $this->load->view('admincp/faqs/add', array(			
			'error' => @$error,
		), true);
		$this->load->view('admincp/ad_layout_view', $this->_data);
	}
	...
	public function edit()
	{
		...
		if ($this->input->post () != "") {
			$keypage = $this->utility_model->find_slugwhere ( $this->table, 'seo_name', $this->input->post ( 'keypage' ) );
			$checkslug = $this->faqs_model->checkSlug($this->input->post ( 'newsID' ), $this->input->post ( 'keypage' ) );
			if($checkslug == TRUE){
				$this->updated ();
				$error = '';
			}else {
		...
		$this->_data ['ad_main_content'] = $this->load->view ( 'admincp/faqs/edit', array (
				'error' => @$error,
