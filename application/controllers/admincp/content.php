 ...
 */
class Content extends Ad_layout
{
	var $table = "tbl_about"; 
	...
	public function index()
	{
		...
		$result = $this->content_model->getAllSearch($arraySearch, $page, $config['per_page']);
		$this->load->library('pagination');
		$config['base_url'] = base_url().'/admincp/content';
		...
		$this->_data['ad_main_content'] = $this->load->view('admincp/content/index', array(
	...
	public function add()
	{
		...
		$this->_data['ad_main_content'] = $this->load->view('admincp/content/add', array(			
			'error' => @$error,
		), true);
		$this->load->view('admincp/ad_layout_view', $this->_data);
	}
		...
		if($insert){
			echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'>
						...
						alert('Bạn đã thêm mới thành công !');
						 location.assign('".base_url('admincp/content.html')."');
					  </script>";
		}else {
			echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'>
						<script>
	...
	public function edit()
	{
		...
		if ($this->input->post () != "") {
			$keypage = $this->utility_model->find_slugwhere ( $this->table, 'seo_name', $this->input->post ( 'keypage' ) );
			$checkslug = $this->content_model->checkSlug($this->input->post ( 'newsID' ), $this->input->post ( 'keypage' ) );
			if($checkslug == TRUE){
				$this->updated ();
				$error = '';
			}else {
		...
		$this->_data ['ad_main_content'] = $this->load->view ( 'admincp/content/edit', array (
				'error' => @$error,
